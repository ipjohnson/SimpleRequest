...
public class ParameterTypeGenerator {
    ...
    private void ConstructParameterInfo(RequestHandlerModel requestModel, ClassDefinition parameterClass) {
        var readonlyType = new GenericTypeDefinition(typeof(IReadOnlyList<>), new[] {
            KnownRequestTypes.IInvokeParameterInfo
        });
        ...
        for (var i = 0; i < requestModel.RequestParameterInformationList.Count; i++) {
            ...
            var parameterInfo = new NewStatement(
                new GenericTypeDefinition(
                    TypeDefinitionEnum.ClassDefinition, KnownRequestTypes.InvokeNamespace + ".Impl", "InvokeParameterInfo", new[] {
            ...
            var field = staticParams.AddField(KnownRequestTypes.IInvokeParameterInfo, "static_" + parameter.Name);
        ...
        if (requestModel.RequestParameterInformationList.Count > 0) {
            ...
            foreach (var parameter in requestModel.RequestParameterInformationList) {
                ...
                caseBlockDefinition.Assign(
                    new StaticCastComponent(field.TypeDefinition, valueParameter)).To("this." + field.Name);
