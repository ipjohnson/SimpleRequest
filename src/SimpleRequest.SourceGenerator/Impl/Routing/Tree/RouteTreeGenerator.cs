...
public class RouteTreeGenerator<T> {
    ...
    public RouteTreeNode<T> GenerateTree(List<Entry> entries) {
        ...
        var route = ProcessEntries("", entries, 0, 0);
    ...
    private RouteTreeNode<T> ProcessEntries(string path, List<Entry> entries, int stringIndex, int wildCardDepth) {
        ...
        if (longestMatch > 0) {
            var matchingNode = ProcessLongMatchingNodes(entries, stringIndex, longestMatch, wildCardDepth);

            if (string.IsNullOrEmpty(path)) {
                return matchingNode;
            }    
                
            var tree = new RouteTreeNode<T>(path,
                new[] { matchingNode  },
